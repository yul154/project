...
public class MyIndexReader {
	...
	public MyIndexReader( String dataType ) throws IOException {
		directory = FSDirectory.open(Paths.get(dataType));  
	...
	public long Length_collection() throws IOException{// get total length of the collection
		long sum = 0;
		...
		while(fieldsIterator.hasNext()){
            ...
            TermsEnum termsEnums = t.iterator();
            BytesRef byteRef = null;
            while((byteRef = termsEnums.next()) != null) {
                String t1 = new String(byteRef.bytes, byteRef.offset, byteRef.length);
                sum = sum + CollectionFreq(t1);
            }
		}
		return sum;
	}
